...
def check_model_compatibility():
    ...
    problematic_models = [
        (
            "mixtral",
            "Mixtral models use a Mixture of Experts architecture that may not be fully compatible with llama-cpp-python on Jetson",
        ),
        (
            "mpt",
            "MPT models have a different architecture that may cause issues with CUDA acceleration on Jetson",
        ),
        (
            "q2_k",
            "Q2_K quantization is too aggressive and may not work well with GPU acceleration",
        ),
    ...
    try:
        with Progress(
            ...
            task = progress.add_task(
                "[cyan]Uninstalling existing llama-cpp-python...", total=None
            )
            ...
            progress.update(
                task,
                description="[cyan]Installing with CUDA support (this may take a while)...",
            )
            env = os.environ.copy()
        ...
        console.print(
            "[green]Successfully compiled llama-cpp-python with CUDA support![/green]"
        )
        return True
    except Exception as e:
        ...
        console.print(
            '[cyan]CUDACXX=/usr/local/cuda/bin/nvcc CMAKE_ARGS="-DGGML_CUDA=on" FORCE_CMAKE=1 pip install llama-cpp-python --force-reinstall --upgrade --no-cache-dir[/cyan]'
        )
        return False
...
def main():
    ...
    if model_name:
        ...
        if issues:
            ...
            console.print(
                "- Llama 3 8B Instruct (Q5_K_M) - Excellent performance with GPU"
            )
            console.print(
                "- Mistral 7B Instruct v0.2 (Q5_K_M) - Great instruction following"
            )
            console.print("- Falcon 7B Instruct (Q5_K_M) - Good performance on Jetson")
            console.print(
                "- CodeLlama 7B Instruct (Q5_K_M) - Specialized for code tasks"
            )
            console.print(
                "- Hermes 3 Llama 3.1 8B (Q5_K_M) - Advanced agentic capabilities"
            )
            ...
            if (
                not console.input("\nContinue with current model anyway? [y/N]: ")
                .lower()
                .startswith("y")
            ):
                console.print(
    ...
    for step_name, step_func in steps:
        ...
        if not success and step_name == "Compile llama-cpp-python with CUDA support":
            if (
                not console.input("\nContinue with other optimizations? [y/N]: ")
                .lower()
                .startswith("y")
            ):
                sys.exit(1)
    ...
    if (
        console.input("\nRun jetson_clocks to maximize performance? [y/N]: ")
        .lower()
        .startswith("y")
    ):
        run_jetson_clocks()
