...
def get_model_info(model_path):
    ...
    gpu_compat = "Good" if any(q in filename for q in ["q5_k_m", "q6_k"]) else \
                 "Medium" if "q4_k_m" in filename else \
                 "Poor" if any(q in filename for q in ["q2_k", "q3_k"]) else "Unknown"
...
def main():
    ...
    try:
        ...
        if current_model and selected_model.resolve() == current_model.resolve():
            console.print(f"[yellow]Already using model: {selected_model.name}[/yellow]")
            return
        
        # Update .env file
        if update_env_file(selected_model):
            console.print(f"[green]Successfully switched to model:[/green] {selected_model.name}")
            ...
            if "hermes" in selected_model.name.lower():
                ...
                console.print("The system has been configured to automatically use the correct format.")
            ...
            console.print("2. For Jetson devices: [cyan]poetry run python demo.py --jetson[/cyan]")
        else:
        ...
        if is_jetson and model_info["gpu_compatibility"] != "Good":
            console.print("[yellow]Note: This model may not work optimally with GPU acceleration on Jetson.[/yellow]")
            console.print("Consider using a model with Q5_K_M or Q6_K quantization for better GPU performance.")
